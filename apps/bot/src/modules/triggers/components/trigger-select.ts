...
function addDeletionFields (embed: EmbedBuilder, trigger: ITrigger): void {
...
function addChannelFields (embed: EmbedBuilder, trigger: ITrigger): void {
...
function addRegexFields (embed: EmbedBuilder, trigger: ITrigger): void {
	if (trigger.matchType === "regex" && trigger.regexFlags) {
		embed.addFields({ name: "Regex Flags", value: trigger.regexFlags, inline: true })
	}
}

function buildTriggerDetailEmbed (trigger: ITrigger): EmbedBuilder {
	...
	addDeletionFields(embed, trigger)
	addChannelFields(embed, trigger)
	addRegexFields(embed, trigger)
...
registerSelectMenu("trigger_select", async (interaction: StringSelectMenuInteraction) => {
	...
	try {
		...
		const embed = buildTriggerDetailEmbed(trigger)
