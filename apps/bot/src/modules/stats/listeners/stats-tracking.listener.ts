...
/**
 * Intervalo que guarda estadísticas de todos los usuarios activos en canales de voz
 */
async function saveAllActiveVoiceStats (): Promise<void> {
	try {
		...
		for (const stats of activeStats) {
			try {
				...
				await saveActiveVoiceStats(stats.userId, stats.guildId, guild, stats)
			} catch (error) {
				statsLogger.error(
 ...
 */
async function cleanOrRestoreSession (
	stats: IUserStats,
	guild: Guild
 ...
 */
async function scanGuildVoiceChannels (guild: Guild): Promise<number> {
 ...
 */
async function restoreVoiceSessions (): Promise<void> {
	try {
		const botInstance = BotInstance.getOrNull()
		if (!botInstance) {
		...
		for (const stats of activeStats) {
			try {
				...
				const result = await cleanOrRestoreSession(stats, guild)
				if (result === "restored") {
					restoredCount++
				} else {
					cleanedCount++
		...
		for (const guild of botInstance.guilds.cache.values()) {
			const count = await scanGuildVoiceChannels(guild)
...
setInterval(saveAllActiveVoiceStats, VOICE_STATS_SAVE_INTERVAL)
statsLogger.info(
...
setTimeout(() => {
	restoreVoiceSessions().catch((error) => {
