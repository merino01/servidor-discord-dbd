...
import { EmbedBuilder, GuildMember, TextChannel, VoiceState } from "discord.js"
...
export class VoiceLogsListener {
	constructor (protected readonly repository: LogsRepository) {}

	private async sendVoiceLogEmbed (
		guildId: string,
		channelId: string,
		embed: EmbedBuilder
	): Promise<void> {
		...
		botEvents.on("voice:join", async (member: GuildMember, voiceState: VoiceState) => {
			...
			try {
				...
				if (voiceConfig?.enabled && voiceConfig.logJoin && voiceConfig.channelId) {
					...
					await this.sendVoiceLogEmbed(member.guild.id, voiceConfig.channelId, embed)
				}
			} catch (error) {
