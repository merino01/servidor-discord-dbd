...
export class MemberLogsListener {
	constructor (protected readonly repository: LogsRepository) {}

	private async sendJoinLogEmbed (member: GuildMember, channelId: string): Promise<void> {
	...
	private async sendLeaveLogEmbed (member: GuildMember | PartialGuildMember, channelId: string): Promise<void> {
		const bot = BotInstance.getOrNull()
		if (!bot) {
			return
		}
	...
	register (): void {
		...
		botEvents.on("member:join", async (member: GuildMember) => {
			try {
				const config = await this.repository.getConfig(member.guild.id)
				...
				if (config.members.channelId) {
					await this.sendJoinLogEmbed(member, config.members.channelId)
				}
			} catch (error) {
		...
		/**
			...
			*/
		botEvents.on("member:leave", async (member: GuildMember | PartialGuildMember) => {
			try {
				const config = await this.repository.getConfig(member.guild.id)
				...
				if (config.members.channelId) {
					await this.sendLeaveLogEmbed(member, config.members.channelId)
				}
			} catch (error) {
