...
import { EmbedBuilder, TextChannel, Message, PartialMessage } from "discord.js"
import { BotInstance } from "@/core/bot-instance"

const logsLogger = logger.child("logs")

function shouldLogMessage (message: Message | PartialMessage): boolean {
...
async function sendDeletedMessageEmbed (data: MessageDeletedEmbedData): Promise<void> {
...
async function sendEditedMessageEmbed (data: MessageEditedEmbedData): Promise<void> {
	const bot = BotInstance.getOrNull()
...
botEvents.on("message:edited", async (
	...
	if (!shouldLogMessage(newMessage) || oldMessage.content === newMessage.content) {return}

	try {
		const config = await LogConfigModel.findOne({ guildId: newMessage.guildId })
		const messagesConfig = config?.messages

		if (messagesConfig?.enabled && messagesConfig.logEdited && messagesConfig.channelId) {
			await sendEditedMessageEmbed({
				channelId: messagesConfig.channelId,
				oldMessage,
				newMessage
			})
