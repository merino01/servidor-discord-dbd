...
export class MessageFormatListener {
	private async validateMessageFormat (message: Message | PartialMessage): Promise<ValidationResult> {
	...
	private async handleInvalidMessage (message: Message, format: ValidationResult["format"]): Promise<void> {
		if (!format) {return}
	...
	register (): void {
		...
		botEvents.on("message:created", async (message: Message) => {
			...
			const validation = await this.validateMessageFormat(message)

			if (!validation.isValid && validation.format) {
				await this.handleInvalidMessage(message, validation.format)
			}
		})
		...
		botEvents.on("message:edited", async (
			...
			const validation = await this.validateMessageFormat(newMessage as Message)

			if (!validation.isValid && validation.format) {
				await this.handleInvalidMessage(newMessage as Message, validation.format)
			}
		})
