...
export class FormatSelectComponent {
	private getFormatStatus (format: IChannelFormat): { text: string; color: number } {
		if (format.deletedAt) {
	...
	private addDeletionFields (embed: EmbedBuilder, format: IChannelFormat): void {
	...
	private buildFormatDetailEmbed (format: IChannelFormat): EmbedBuilder {
		...
		this.addDeletionFields(embed, format)
	...
	register (): void {
		registerSelectMenu("format_select", async (interaction: StringSelectMenuInteraction) => {
			...
			try {
				...
				await interaction.update({
					embeds: [this.buildFormatDetailEmbed(format)],
					components: interaction.message.components
				})
			} catch (error) {
